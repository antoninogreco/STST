...
class ColorTransfer:
    ...
    def pdf_tranfer(self, img_arr_in=None, img_arr_ref=None, dtype='float'):
        ...
        if dtype=='int':
            reshape_arr_in /= 255.0 
            reshape_arr_ref /= 255.0 
        # pdf transfer
        ...
        if dtype=='int': reshape_arr_out = (255.0 * reshape_arr_out).astype("uint8")
...
class Regrain:
    ...
    def regrain(self, img_arr_in=None, img_arr_col=None,dtype='float'):
        ...
        if dtype=='int': img_arr_out = (255.0 * img_arr_out).astype("uint8")
        return img_arr_out
...
def color_transfer(img_arr_in,img_arr_ref):
    PT = ColorTransfer()
    RG = Regrain()
